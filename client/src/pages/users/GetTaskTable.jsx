  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const res = await axios.get(`/api/v1/task/get/${userId}`);
        setTasks(res.data);
      } catch (err) {
...
const handleStatusChange = async (taskId, newStatus) => {
  const confirm = window.confirm("Are you sure you want to update the task status?");
  if (!confirm) return;

  try {
    const res = await axios.put(`/api/v1/task/update/${taskId}`, { status: newStatus });
    if (res.status === 200) {
      // Refresh tasks
      setTasks((prevTasks) =>
        prevTasks.map((task) =>
